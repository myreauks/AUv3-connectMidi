//
//  connectMidiMultipleExtensionDSPKernel.hpp
...
#import "connectMidiMultipleExtension-Swift.h"
#import "connectMidiMultipleExtensionParameterAddresses.h"

constexpr uint16_t kMaxVelocity = std::numeric_limits<std::uint16_t>::max();

/*
 connectMidiMultipleExtensionDSPKernel
 As a non-ObjC class, this is safe to use from render thread.
 */
class connectMidiMultipleExtensionDSPKernel {
public:
    ...
    // Add a case for each parameter in connectMidiMultipleExtensionParameterAddresses.h
    void setParameter(AUParameterAddress address, AUValue value) {
        switch (address) {
            case connectMidiMultipleExtensionParameterAddress::midiNoteNumber:
                mNextNoteToSend = (uint8_t)value;
                break;
            case connectMidiMultipleExtensionParameterAddress::sendNote:
    ...
    void handleMIDIEventList(AUEventSampleTime now, AUMIDIEventList const* midiEvent) {
        /*
         ...
         auto thisObject = static_cast<connectMidiMultipleExtensionDSPKernel *>(context);
